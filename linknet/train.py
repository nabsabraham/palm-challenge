...
model = LinkNet(n_classes=1)
model.load_state_dict('checkpoint.pt')
x_val, y_val = next(iter(val_loader))
preds = model(x_val.cuda())
preds = preds.cpu().detach()
grid_gt = vutils.make_grid(y_val, nrow=2)
grid_pred = vutils.make_grid(preds, nrow=2)
grid_img = vutils.make_grid(x_val, nrow=2)
...
plt.figure()
plt.imshow(grid_gt.permute(1,2,0) - grid_pred.permute(1,2,0))
